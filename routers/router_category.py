...
async def get_category():
    ...
    for product in products:
        resultat.append(product.category)
    return resultat

@router.get('/{category}', response_model=List[schemas_dto.Product])
async def get_product_by_category(category: str):
    ...
    if resultat!=[]:
        ...
        raise HTTPException(status_code=404, detail="Category {category} not found")